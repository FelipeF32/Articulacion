 ...
 * query ?estado=opendiete&usaurioId=1&pagina=1&limite=10
 */
...
const getAllPedidos = async (req, res) => {
    try {
        ...
        const where = {};
        if (estado) where.estado = estado;
        if (usuarioId) where.usarioId = usuarioId;
...
const actualizarEstadoPedido = async (req,res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            message: 'Error al asctualizar el estado del pedido',
            error: error.messaga
        });
    }
};
...
/**
 * Obtener estadisticas de pedido
...
const getEstadisticasPedidos = async (req,res) => {
    try {
        ...
        const pedidosHoy = await Pedido.count({
            where: {
                createAt: { [open.gte]: hoy } //pedidos ultimos 7 dias
            }
        });
        ...
        res.json({
            success: true,
            message: 'Estados de los pedidos obtenidos',
            data: {
                totalPedidos,
                pedidosHoy,
